...
Page({
	...
	getTodayData() {
		...
		postData.compute.totalCount[2].create_time = ['between', [new Date(new Date().toLocaleDateString()).getTime() /
			1000,
			new Date(new Date().toLocaleDateString()).getTime() + 24 * 60 * 60 - 1
		]]
		const callback = (res) => {
	...
	getReward(isNew) {
		...
		const callback = (res) => {
			...
			if (res.info.data.length > 0) {
				...
				var todayRatioMoney = parseFloat(self.data.todayMoney) * (1 - parseFloat(self.data.userInfoData.ratio) / 100) *
					(parseFloat(self.data.rewardData.alliance_ratio) / 100);
				var ratio = (1 - parseFloat(self.data.userInfoData.ratio) / 100) * (parseFloat(self.data.rewardData.alliance_ratio) /
					100) - (parseFloat(self.data.userInfoData.ratio) / 100) * (parseFloat(self.data.rewardData.alliance) / 100);
	...
	bindTimeChange: function(e) {
		...
		if (self.data.endTimestap && self.data.startTimestap) {
			if (self.data.currentId != 1) {
				...
				self.data.searchItemTwo.create_time = ['between', [self.data.startTimestap, self.data.endTimestap]];
			}
			...
			if (self.data.currentId != 1) {
				...
				self.data.searchItemTwo.create_time = ['>', self.data.startTimestap];
			}
		} else {
			if (self.data.currentId != 1) {
				self.data.searchItem.create_time = ['<', self.data.endTimestap];
			} else {
				self.data.searchItemTwo.create_time = ['<', self.data.endTimestap];
			}
